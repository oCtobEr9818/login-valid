...
const PnSummaryFEX0102 = () => {
  const [pnFEX0102, setPnFEX0102] = useState([]);
  ...
  const getPnListDatas = async () => {
    try {
      ...
      for (const element of pnJSON) {
        if (element.pn_name === "FEX0102") {
          setPnFEX0102(element);
        }
      }

      const deviceResponse = await axiosPnListApi.get(`/pn/1/sn/1/device/all`);
