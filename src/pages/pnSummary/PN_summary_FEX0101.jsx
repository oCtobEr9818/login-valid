...
const PnSummaryFEX0101 = () => {
  const [pnFEX0101, setPnFEX0101] = useState([]);
  ...
  const getPnListDatas = async () => {
    try {
      ...
      for (const element of pnJSON) {
        if (element.pn_name === "FEX0101") {
          setPnFEX0101(element);
        }
      }

      const deviceResponse = await axiosPnListApi.get(`/pn/2/sn/2/device/all`);
